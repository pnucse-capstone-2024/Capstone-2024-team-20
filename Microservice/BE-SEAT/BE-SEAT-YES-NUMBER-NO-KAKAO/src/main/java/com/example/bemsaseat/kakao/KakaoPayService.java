...
public class KakaoPayService {
    ...
    public KakaoReadyResponse kakaoPayReady(List<SeatDetail> seatDetails, String email) {
        ...
        if (!seatDetails.isEmpty()) {
            // 첫 번째 아이템 처리
            SeatDetail firstSeat = seatDetails.get(0);
            itemNames.append(firstSeat.getEventName())
                    .append("-")
                    .append(firstSeat.getSection())
                    .append("-")
                    .append(firstSeat.getSeatNumber())
                    .append("-")
...
//        String approval_url = "http://localhost:8082/seat/kakao/success";
        ...
        try {
            ...
            // 이게 있어야 DTO 저장된다
            purchaseResponseDTO = new PurchaseResponseDTO(kakaoReadyResponse, email, 0);
